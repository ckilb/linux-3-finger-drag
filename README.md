...
* A fresh touch is **withheld** from the compositor until classified: a lone finger with intentional pointer motion goes live after `probeDelay` (default 15 ms — ordinary pointer motion never feels delayed), a stationary or ambiguous 1-3 finger touch waits out `entryDebounce` (default 50 ms), and 4+ fingers goes live instantly. Real fingers land and lift asynchronously; judging a touch frame-by-frame (the naive approach) leaks phantom taps and misreads gestures.
...
| `probeDelay` | `15` | Length of time (in milliseconds) a lone finger with intentional pointer motion is withheld before going live. A stationary finger remains protected until `entryDebounce`, preventing a late-landing 3-finger tap from leaking as a right-click. |
| `dragStartThreshold` | `0.5` | How far (in **millimetres** of physical finger travel) a 3-finger touch must move before it becomes a drag. Below this it stays a **tap**, which middle-clicks (the macOS/libinput 3-finger tap). This deadzone is what stops a resting finger's natural jitter from randomly turning a middle-click tap into a stray left-button drag. Larger = more jitter-proof but a drag needs a bigger nudge to start; smaller = twitchier. |
| `pressGrace` | *(inert)* | Deprecated. A drag no longer presses on any timer — the button is pressed only by real motion (see `dragStartThreshold`), so a stationary 3-finger touch is always a tap. Still accepted in the config file for backward compatibility, but has no effect. |
...
* **3-finger tap (middle click) doesn't always register / sometimes drags instead** — your resting fingers are jittering past the drag-start deadzone. Raise `dragStartThreshold` (e.g. to `0.8` or `1.0` mm) so a tap has to hold stiller to count as stationary. **Drag won't start until I move a lot?** Lower it.
